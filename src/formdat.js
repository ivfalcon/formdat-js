...
class Formdat {
   ...
   */
  static #clean(detachedDate, lang = 'en') {
    return detachedDate.filter((el) => {
      const isDateElement = this.#months[lang].some(monthArr => monthArr.some((month) => (month === el.toLowerCase() || (/^\d{1,4}$/).test(el))));
   ...
   */
  static #getUnifiedDate(unifiedDate, type = 'yearFirst') {
    const typeOfDate = {
   ...
   * @returns array containing the date elements (year, month and day)
   */
  static #dateDetacher(date, type = 'yearFirst', lang = 'en') {
    const result = [];
      ...
      for (let i = 0; i < el.length; i++) {
        if (isDigit && (/\D/).test((el).charAt(i))) {
          if (unifiedDatePattern.test(el.substring(init, i))) {
            result.push(...this.#getUnifiedDate(el.substring(init, i), type));
          } else {
        ...
        if (el.length - 1 === (i)) {
          if (unifiedDatePattern.test(el.substring(init, el.length))) {
            result.push(...this.#getUnifiedDate(el.substring(init, el.length), type));
          } else {
   ...
   */
  static #monthSearch(month, lang = 'en') {
    let found = false;
    let cont = 0;
    let j = 0;
    let monthPos = '';

    while (!found && cont < this.#months[lang].length) {
      j = 0;
      while (!found && j < this.#months[lang][cont].length) {
        found = this.#months[lang][cont].indexOf(month) > -1 ? true : false;
        if (found) {
          monthPos = cont + 1;
        }
        j++;
   ...
   */
  static #addZeros(year, month, day) {
   ...
   */
  static mdy(date = '', lang = 'en') {
    if (typeof date !== 'string') {
      return false;
    }
    let year, month, day;
    const result = this.#dateDetacher(date, 'yearLast', lang);
    if (result.length === 3) {
      [month, day, year] = [...result];
      if (!(/\d/).test(month)) {
        month = this.#monthSearch(month, lang);
      }
      [year, month, day] = this.#addZeros(year, month, day);
      return this.#isValidDate(year, month, day) ? ([year, month, day].join('/')) : false;
    }
    else return false;
  }
   ...
   * @returns {(string|boolean)} the date in format YYYYY/MM/DD
   */
  static ydm(date = '', lang = 'en') {
    if (typeof date !== 'string') {
      return false;
    }
    let year, month, day;
    const result = this.#dateDetacher(date, 'yearFirst', lang);
    if (result.length === 3) {
  ...
  /**
     ...
     * @returns {(string|boolean)} the date in format YYYYY/MM/DD
     */
  static myd(date = '', lang = 'en') {
    if (typeof date !== 'string') {
      return false;
    }
    
    let year, month, day;
    const result = this.#dateDetacher(date, 'yearMid', lang);
    if (result.length === 3) {
